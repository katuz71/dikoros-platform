...
def _chat_info_fallback_answer(user_message: str) -> str:
    ...
    if any(x in t for x in [
        "персональн", "данн", "даних", "данные", "конфиденц", "конфіденц", "privacy",
        "политик", "політик", "удалить", "удален", "видалити", "видален", "аккаунт", "акаунт"
    ]):
        return (
            "DikorosUA може обробляти персональні дані, необхідні для роботи сервісу: ім’я, прізвище, номер телефону, email, місто, відділення поштового оператора або адресу доставки.\n\n"
            "Також можуть оброблятися історія замовлень, склад кошика, спосіб оплати, спосіб доставки, статуси замовлень, бонусний баланс, відгуки, обране, SMS-коди, статус підтвердження телефону, технічні ідентифікатори входу через Google, push-token, версія застосунку, тип пристрою, технічні журнали помилок і події аналітики.\n\n"
            "Дані використовуються для реєстрації, входу, оформлення й доставки замовлень, передачі замовлення в CRM, зв’язку з клієнтом, підтримки, повернень, бонусної програми, аналітики та стабільної роботи застосунку.\n\n"
            "DikorosUA не продає персональні дані третім особам. Дані можуть передаватися службам доставки, платіжним сервісам, банкам, OneBox, SMS-сервісам, сервісам підтримки, хостингу, Google/Firebase, Meta та іншим технічним підрядникам лише в обсязі, необхідному для роботи сервісу.\n\n"
            "Акаунт можна видалити у застосунку: Профіль → Налаштування → Видалити акаунт."
        )

    if any(x in t for x in [
        "міжнарод", "международ", "international", "тамож", "митниц", "пошлин", "мито", "налог", "податк"
    ]):
        return (
            "Міжнародні відправлення DikorosUA здійснюються після 100% передплати: вартість товару + вартість доставки.\n\n"
            "Укрпоштою можна відправляти всі категорії товарів. Новою поштою міжнародна відправка можлива тільки для БАДів.\n\n"
            "Орієнтовна вартість доставки Укрпоштою — від 400 до 2500 грн залежно від країни, ваги та об’єму посилки. Новою поштою — від 300 грн. Фінальна вартість розраховується після збирання та пакування замовлення.\n\n"
            "Середній строк доставки Укрпоштою — близько 3 тижнів, але він залежить від країни, митниці та поштового оператора. Нова пошта в середньому — 3–5 днів.\n\n"
            "Кожна країна має власні митні правила, податки, мито та обмеження. DikorosUA не впливає на рішення митниці іншої країни. Відповідальність за законність отримання товару у своїй країні несе покупець."
        )

    if any(x in t for x in ["оферт", "договір", "договор", "умов", "услов"]):
        return (
            "Договір оферти є публічною пропозицією DikorosUA укласти договір купівлі-продажу товарів дистанційним способом.\n\n"
            "Договір застосовується до замовлень, оформлених через застосунок Dikoros, сайт DikorosUA або через менеджера. Оформлення замовлення, підтвердження або оплата товару означає прийняття покупцем умов оферти.\n\n"
            "Покупець має надати достовірні контактні дані та дані доставки. Якщо товар відсутній або його неможливо відправити, менеджер повідомляє покупця та може запропонувати заміну, очікування надходження або скасування замовлення.\n\n"
            "Спори вирішуються шляхом переговорів, а якщо домовитися неможливо — відповідно до законодавства України."
        )
    ...
    if any(x in t for x in [
        "оферт", "договір", "договор", "умов", "услов",
        "політик", "политик", "конфіденц", "конфиденц",
        "персональн", "privacy", "видален", "видалити", "удален", "удалить",
        "акаунт", "аккаунт", "міжнарод", "международ", "international",
        "митниц", "тамож", "податк", "налог", "мито", "пошлин",
    ]):
        return CHAT_KNOWLEDGE.get("legal_and_contacts") or "Юридичні умови, оферту, політику конфіденційності та контакти можна переглянути у застосунку в юридичних розділах або уточнити у менеджера."
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        legal_info_keywords = [
            "оферт", "договір", "договор", "умов", "услов",
            "персональн", "данн", "даних", "данные",
            "конфиденц", "конфіденц", "privacy", "политик", "політик",
            "удалить", "удален", "видалити", "видален", "аккаунт", "акаунт",
            "міжнарод", "международ", "international", "тамож", "митниц",
            "пошлин", "мито", "налог", "податк",
        ]
        if any(keyword in normalized_message for keyword in legal_info_keywords):
            text = _chat_info_fallback_answer(user_message)
            return ChatResponse(
                message=text,
                reply=text,
                products=[],
                items=[],
                quick_replies=_chat_info_quick_replies(),
                session_id=session_id,
            )
